...
from solver import PuzzleSolver, FieldState
...
class Graph:
    ...
    def _calculate_color(self, n: int):
    ...
    def _set_edge_color_and_width(self, n1, n2, ed):
        """Sets color and width for edges."""
        if self.graph.nodes[n1][NUMBER] != 0 and self.graph.nodes[n1][NUMBER] == self.graph.nodes[n2][NUMBER]:
    ...
    def draw(self, figsize: tuple = (10, 10)) -> None:
        """Draws the graph."""
...
class MasterGraph(Graph):
    ...
    def solve_puzzle(self):
        ...
        state = FieldState.from_list_to_state([[self.graph.nodes[(i, j)][NUMBER] for j in range(self._cols)] for i in range(self._rows)])
        solver = PuzzleSolver(state)
        solver.solve()
        solved_matrix = [[solver.field_state.get_state((i, j)) for j in range(self._cols)] for i in range(self._rows)]
        self.set_numbers(solved_matrix)  # Update the graph with solved numbers
...
def set_walls_for_neighbors(graph: MasterGraph) -> MasterGraph:
    for n1, n2, data in graph.graph.edges(data=True):
        if graph.graph.nodes[n1][NUMBER] != 0 and graph.graph.nodes[n2][NUMBER] != 0:
            if graph.graph.nodes[n1][NUMBER] != graph.graph.nodes[n2][NUMBER]:
                data[WALL] = True
...
def run():
    ...
    graph.draw()
    graph.solve_puzzle()
    graph = set_walls_for_neighbors(graph)
    graph.draw()
    plt.show()
